...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        f"👋 Добро пожаловать, {user_name}! Я — AIPolytech, твой персональный цифровой помощник в мире Московского Политеха.\n\n"
        "Моя задача — сделать твою учёбу и жизнь в университете проще и удобнее. Я помогу тебе ориентироваться во всех официальных сервисах и находить нужную информацию за считанные секунды. 🤖✨\n\n"
        "💬 Можно задать вопрос текстом: напиши свой вопрос в свободной форме, и наша LLM постарается дать ответ.\n\n"
        "⏳ Важно: иногда ответ от модели может приходить дольше обычного из-за высокой нагрузки на сервис.\n\n"
        "Что я умею:\n"
        "📅 Расписание: покажу расписание занятий твоей группы, шаг за шагом.\n\n"
        "📚 Программы обучения: помогу сориентироваться в направлениях бакалавриата по 9 факультетам.\n\n"
        "📋 Услуги МФЦ: расскажу, как получить справку, восстановить студенческий билет или оформить документы.\n\n"
        "💰 Стипендии: подробно опишу все виды стипендий — от академической до президентской.\n\n"
        "🏘 Общежития: найду адрес, расскажу о типах комнат и стоимости проживания.\n\n"
        "📌 Студпроекты: покажу актуальные студенческие проекты и инициативы.\n\n"
        "🎉 Мероприятия: расскажу о предстоящих событиях и праздниках в университете.\n\n"
        "📞 Контакты: помогу найти контакты нужного человека или подразделения.\n\n"
        "Моя информация всегда основана на данных с официального сайта университета.\n\n"
        "Полезные ссылки:\n\n"
        "🌐 Официальный сайт: mospolytech.ru\n\n"
        "📢 Новости и анонсы: Telegram-канал @mospolytech\n\n"
        "💬 Пообщаться со студентами: общий чат @mospolychat\n\n"
        "❓ Вопросы по поступлению: чат для абитуриентов @mospolyab_chat\n\n"
        "Просто выбери нужный раздел в меню ниже или задай вопрос своими словами, и я постараюсь помочь! 👇😊"
    )
...
🎓 Абитуриенту - информация о поступлении
📚 Студенту - информация для студентов
📰 Новости - последние новости МосПолитеха
❓ Помощь - справка
...
async def handle_programs_redirect(message: types.Message):
    ...
    # Эта функция больше не нужна, так как программы теперь обрабатываются отдельным маршрутизатором
    pass


# СТАРЫЙ ОБРАБОТЧИК (экспортирован на случай необходимости)
@router.message(F.text == "📚 Направления обучения")
async def handle_directions(message: types.Message):
    """Информация о направлениях обучения (старая версия - не используется)"""
    directions_text = """
...
async def handle_student(message: types.Message):
    ...
    logger.info("📚 Вход в раздел Студенту")
...
"""
    ...
    logger.info(f"🎖️ Клавиатура содержит {len(keyboard.keyboard)} строк с кнопками")
    
    # Проверяем наличие кнопки Военнообязанным
    has_military = any(
        button.text == "🎖️ Военнообязанным"
        for row in keyboard.keyboard
        for button in row
    )
    logger.info(f"🎖️ Кнопка Военнообязанным: {'НАЙДЕНА' if has_military else 'НЕ НАЙДЕНА'}")
...
"""
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🎓 Вопросы об учёбе")],
...
Q: Как мне получить расписание?
A: Нажми кнопку "📚 Студенту" → "📅 Расписание занятий" и введи номер группы.

Q: Как узнать номер аудитории для занятия?
A: Номер аудитории указан в расписании. Дополнительно смотри схемы-планы на входе в корпуса.

Q: Где находится учебный отдел?
A: Корпус 1, 4 этаж. Режим работы: Пн-Пт 09:00-17:00

Q: Как восстановить пароль от портала?
A: Перейди на https://portal.mospolytech.ru/reset и выполни инструкции по восстановлению.

Q: Где получить справку для банка/работодателя?
A: Деканат факультета, кабинет "Студенческие дела", или онлайн через портал.

Q: Какой размер стипендии?
A: Базовая - 2,500 руб., повышенная - 5,000 руб., персональная - до 10,000 руб.

Q: Как подать документы на общежитие?
A: Через портал студента или в отдел жилищного обеспечения (корпус 1, подвал).

Q: Когда пересдача экзаменов?
A: Обычно в конце сессии. Уточни у куратора или в деканате.

Q: Могу ли я перевестись на другую специальность?
A: Да, обратись в деканат. Возможны переводы в конце первого и второго курса.

Q: Как получить справку об обучении?
A: Студ. отдел выдает справки в день обращения. Требуется зачетная книжка и паспорт.
"""
...
async def handle_library(message: types.Message):
    """Информация о библиотеке МосПолитеха"""
    library_text = """
📖 Библиотечно-информационный центр МосПолитеха

Современный библиотечный комплекс, предоставляющий неограниченный доступ к литературе и информации для всех категорий читателей.

🎯 Что мы предоставляем:
✅ Доступ к компьютерам с Интернетом
✅ Электронный каталог БИЦ
✅ Информационные и полнотекстовые ресурсы
✅ Базы данных научного цитирования
✅ Консультационную помощь в поиске литературы
✅ Временное пользование изданиями
✅ Продление сроков пользования
✅ Библиографические справки

🌐 Электронные ресурсы (ЭБС):
• Образовательная платформа «Юрайт»
• Электронно-библиотечная система IPR SMART (Премиум)
• Электронно-библиотечная система «Издательство Лань»
• Ресурсы издательства Springer Nature

📍 Локации:

🏢 Библиотека «На Автозаводской»
   Ул. Автозаводская, 16, корп. 2, ауд. 2702, 2706
   📞 +7 (495) 223-05-23, доб. 2270

🏢 Библиотека «На Корчагина»
   Ул. П. Корчагина, 22, ауд. 114
   📞 +7 (495) 223-05-23, доб. 3210

🏢 Библиотека «На Прянишникова»
   Ул. Прянишникова, 2а, 7 этаж
   📞 +7 (495) 223-05-23, доб. 4106

🏢 «Новая библиотека»
   Ул. Большая Семеновская, 38 (ауд. Н-416)
   Ул. Автозаводская, 16 (ауд. 4905)

🕐 Режим работы:

Библиотеки (пн-чт): 10:00 - 20:00
Библиотеки (пт): 10:00 - 18:00
Новая библиотека (пн-пт): 10:00 - 22:00
Выходные: сб, вс
Санитарный день: последняя пятница месяца

💡 Для доступа к электронным ресурсам достаточно интернета и пароля. Регистрация возможна с любого устройства!
"""
    ...
    await message.answer(library_text, parse_mode="HTML", reply_markup=keyboard)


@router.message(F.text == "🎖️ Военнообязанным")
async def handle_military(message: types.Message):
    """Обработчик раздела 'Военнообязанным'"""
    import json
    
    try:
        # Загружаем данные из файла military.json
        with open('docs/military.json', 'r', encoding='utf-8') as f:
            military_data = json.load(f)
        
        military_text = f"""
{military_data.get('section_title', '🎖️ Военнообязанным и призывникам')}

Информация о военной подготовке и обязательствах студентов.
...
"""
        ...
        keyboard = ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🎓 Первокурсникам")],
                [KeyboardButton(text="📞 Контакты и режим работы")],
                [KeyboardButton(text="◀️ Назад")],
                [KeyboardButton(text="🏠 Главное меню")],
            ],
            resize_keyboard=True
        ...
        await message.answer(military_text, reply_markup=keyboard)
    
    except Exception as e:
        logger.error(f"Ошибка в handle_military: {e}")
        await message.answer(
            "❌ Ошибка при загрузке информации. Пожалуйста, попробуйте позже.",
            reply_markup=get_student_menu_keyboard()
        )


@router.message(F.text == "🎓 Первокурсникам")
async def handle_military_freshmen(message: types.Message):
    """Информация для первокурсников о военной подготовке"""
    import json
    ...
    try:
        ...
        freshmen_section = next(
            (item for item in military_data['main_menu'] if item['id'] == 'freshmen'),
            None
        )
        
        if freshmen_section:
            text = f"<b>🎓 {freshmen_section['name']}</b>\n\n"
            
            for subitem in freshmen_section['content']['submenu_items']:
                text += f"<b>{subitem['name']}</b>\n"
                
                if subitem['type'] == 'text':
                    text += f"{subitem['text']}\n\n"
                elif subitem['type'] == 'list':
                    for item in subitem.get('items', []):
                        text += f"• {item}\n"
                    text += "\n"
                elif subitem['type'] == 'table':
                    text += "Общежитие    →    Военкомат    →    Адрес\n"
                    for row in subitem.get('data', []):
                        text += f"• {row['dorm']} → {row['office']} → {row['address']}\n"
                    text += "\n"
            
            keyboard = ReplyKeyboardMarkup(
                keyboard=[
                    [KeyboardButton(text="🎖️ Военнообязанным")],
                    [KeyboardButton(text="◀️ Назад")],
                    [KeyboardButton(text="🏠 Главное меню")],
                ],
                resize_keyboard=True
        ...
        logger.error(f"Ошибка в handle_military_freshmen: {e}")
        await message.answer("❌ Ошибка при загрузке информации.")


@router.message(F.text == "📞 Контакты и режим работы")
async def handle_military_contacts(message: types.Message):
    """Контакты управления мобилизационной подготовки"""
    import json
    ...
    try:
        ...
        contacts_section = next(
            (item for item in military_data['main_menu'] if item['id'] == 'contacts'),
            None
        )
        
        if contacts_section:
            text = f"<b>{contacts_section['name']}</b>\n\n{contacts_section['text']}"
        ...
        logger.error(f"Ошибка в handle_military_contacts: {e}")
        await message.answer("❌ Ошибка при загрузке информации.")
