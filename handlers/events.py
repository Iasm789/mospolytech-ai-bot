...
from utils.message_manager import send_or_edit_message
...
"""
    ...
    await send_or_edit_message(
        message, events_text, reply_markup=get_events_menu_keyboard(), parse_mode="HTML"
    )
...
async def show_all_events(message: types.Message, page: int = 0):
    ...
    if not all_events:
        await send_or_edit_message(
            message, "❌ Мероприятия не найдены.", reply_markup=get_events_menu_keyboard()
        )
        return
    ...
    # Заголовок
    text = "<b>📋 Все мероприятия МосПолитеха</b>\n"
    ...
    text += "<i>Нажми на название события, чтобы увидеть полную информацию</i>"
...
async def show_category_events(
    message: types.Message, category: str, category_name: str, page: int = 0
):
...
async def cancel_search(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await send_or_edit_message(
        message, "❌ Поиск отменён.", reply_markup=get_events_menu_keyboard()
    )
...
async def handle_search_page_navigation(query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if page > 0:
            pagination_row.append(
                InlineKeyboardButton(
                    text="⬅️ Назад", callback_data=f"search_page:{search_query}:{page-1}"
                )
            )
        ...
        if page < total_pages - 1:
            pagination_row.append(
                InlineKeyboardButton(
                    text="Вперёд ➡️", callback_data=f"search_page:{search_query}:{page+1}"
                )
            )
...
async def back_to_main_menu_from_events(message: types.Message):
    ...
    await send_or_edit_message(
        message, "📋 <b>Главное меню</b>", reply_markup=get_main_menu_keyboard(), parse_mode="HTML"
    )
...
async def cmd_reload_events(message: types.Message):
    ...
    try:
        ...
        status_msg = await message.answer(
            "⏳ Перезагружаю мероприятия из JSON файла...", parse_mode="HTML"
        )
        ...
        if success:
            ...
            result_text = (
                "✅ <b>Мероприятия успешно перезагружены!</b>\n\n" "📊 <b>Статистика:</b>\n"
            )
        ...
        await message.answer(
            f"❌ <b>Ошибка при перезагрузке!</b>\n\n" f"Детали: {str(e)[:200]}", parse_mode="HTML"
        )
