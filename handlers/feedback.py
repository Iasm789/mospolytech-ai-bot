...
📋 Тип: {feedback_type_names.get(data.get('feedback_type'), data.get('feedback_type'))}
{"📌 Заголовок: " + (data.get('title', '')[:50] if data.get('title') else "—") + "\n" if data.get('title') else ""}📝 Сообщение: {data.get('message', '')[:150]}...
{"⭐ Оценка: " + ("★" * data.get('rating')) if data.get('rating') else ""}
